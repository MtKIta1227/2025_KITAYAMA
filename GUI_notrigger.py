...
# -*- coding: utf-8 -*-
"""
pump_on_off_deltaA_with_timing_gui_v2.py
────────────────────────────────────────────────────────
元のスクリプトを PyQt5 ベースの GUI アプリケーションに変換（改訂版）。

[v2での改善点]
• 測定完了後、分光器の接続を維持したまま、パラメータを変更して
  連続で再測定できるようにフローを改善。
• 積分時間はポンプOFF測定を開始するたびに最新の値が適用される。
• 接続を根本的にやり直すための「接続をリセット」ボタンを追加。
"""
...
# ───────────── 初期設定 ─────────────
N_SCANS_DEFAULT = 200  # ポンプ ON / OFF それぞれのショット数
INTEG_MS_DEFAULT = 5.0  # 積分時間 [ms]
...
class MainWindow(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Pump-Probe ΔA Measurement v2.0")
        ...
        self.n_scans_spinbox.setRange(1, 10000)
        self.n_scans_spinbox.setValue(N_SCANS_DEFAULT)
        self.integ_ms_spinbox = QDoubleSpinBox()
        self.integ_ms_spinbox.setRange(1.0, 10000.0)
        self.integ_ms_spinbox.setValue(INTEG_MS_DEFAULT)
        self.integ_ms_spinbox.setDecimals(1)
        self.integ_ms_spinbox.setSingleStep(0.1)
    ...
    def _set_ui_state(self, state: str):
        if state == "initial":
            ...
            self.pump_on_button.setEnabled(False)
            self.n_scans_spinbox.setEnabled(True) # 再測定のために有効化
            self.integ_ms_spinbox.setEnabled(True) # 再測定のために有効化
    ...
    def on_acquisition_error(self, error_msg):
        ...
        self.thread.quit()
        self.thread.wait()
        self._set_ui_state("ready_for_off") # エラー後も再試行可能に
    ...
    def process_and_display_results(self):
        ...
        self._set_ui_state("ready_for_off") # 再測定待機状態へ
    ...
    def plot_results(self, delta_a, dt_off, dt_on):
        ...
        self.ax_a.set_xlabel("Wavelength (nm)")
        self.ax_a.set_ylabel("ΔA")
