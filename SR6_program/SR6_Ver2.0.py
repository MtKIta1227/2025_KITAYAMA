...
class OceanDirectApp(QMainWindow):
    def __init__(self):
        ...
        self.left_layout.addLayout(connection_layout)
        self.left_layout.addLayout(controls_layout)
        self.left_layout.addWidget(self.acquire_button)
        self.left_layout.addWidget(self.plot_widget, 1)

        self.right_widget = QWidget()
        self.right_layout = QVBoxLayout(self.right_widget)
        self.data_list = QTreeWidget()
        self.data_list.setHeaderLabels(["測定データ"])
        self.right_layout.addWidget(QLabel("仮保存スペクトルリスト"))
        self.right_layout.addWidget(self.data_list)
        ...
        self.data_list.itemClicked.connect(self.load_selected_spectrum)
    ...
    def acquire_spectrum(self):
        ...
        try:
            ...
            if self.wavelengths and spectrum_data:
                ...
                self.spectra_data.append((label, spectrum_data))
    ...
    def load_selected_spectrum(self, item):
        label = item.text(0)
        for name, data in self.spectra_data:
            if name == label:
